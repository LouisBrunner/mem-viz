...
func (me *parser) addCache(parent *contracts.MemoryBlock, cache subcontracts.Cache, label string, offset subcontracts.Address) (*contracts.MemoryBlock, *contracts.MemoryBlock, error) {
	...
	err = me.parseMappings(frame, mappings)
	if err != nil {
		return nil, nil, err
	}
	if okV1 {
	...
	// TODO: somehow conflicts with LINKEDIT?
	// sigBlock, err := me.createBlobBlock(frame, "CodeSignatureOffset", header.CodeSignatureOffset, "CodeSignatureSize", header.CodeSignatureSize, "Code Signature")
	// if err != nil {
	// 	return nil, nil, err
	// }
	// // // FIXME: might be worth refactoring this in something nicer
	// if sigBlock != nil {
	// 	const sigMagic = 0xfade0cc0
	// 	abc := struct {
	// 		Magic uint32
	// 	}{}
	// 	err := commons.Unpack(header.CodeSignatureOffset.AddBase(uintptr(frame.offsetFromStart+frame.parentStruct.ParentOffset)).GetReader(frame.cache, 0, me.slide), &abc)
	// 	if err != nil {
	// 		return nil, nil, err
	// 	}
	// 	if abc.Magic == sigMagic {
	// 		return nil, nil, fmt.Errorf("code signature is invalid (magic %#x vs %#x)", abc.Magic, sigMagic)
	// 	}
	// }
	if okV1 {
	...
	err = me.parseImageTexts(frame, imgTexts)
	if err != nil {
		return nil, nil, err
	}
	if okV1 {
	...
	err = me.parseMappingsWithSlide(frame, mappingsWithSlide)
	if err != nil {
		return nil, nil, err
	}
	...
	// TODO: it's often wrong, conflicting with other parts of memory, CodeSignature, Mapping 4, etc (especially on amd64)
